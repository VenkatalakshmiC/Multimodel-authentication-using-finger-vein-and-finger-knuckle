...
for i in range(nfolders-4):
    ...
    for j in range(7,7+Images_Per_vein,1):
        print(j)
        fileName = files[j]
...
for k in threshold:
    ...
    mismatch_cnt = 0   
    m=0
    for pp in range(nfolders-4):
        
        for ii in range(7,7+Images_Per_vein,1):
    ...
    frr[kk] = (mismatch_cnt/ (No_Of_persons*Images_Per_vein))*100                
    tsr[kk] = (match_cnt/ (No_Of_persons*Images_Per_vein))*100                   
    kk=kk+1   
